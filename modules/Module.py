...
def protoZero(INPUT_FILE, TIME, POINTS, COEFF, MAX_LIPID, PROTO_TYPE, DIVISION, LIPID_EXP, RHO, DELTA, species, frequences, reactions, catalysis, events):
        ...
        ### Poi leggo le reazioni e le loro frequenze ###                
            ...
            for i in columns:
                if i == '>':
                    ...
                    if len(reactants) == 1 and reactants[0].isdigit() and PROTO_TYPE == 3:
                        frequence = float(i)/(RHO*DELTA)
                    else:
                        frequence = float(i)
