...
#    TRAJECTORIES = readTrajectories()
    ...
    RHO = readRho()         # Densità del lipide
    DELTA = readDelta()     # Spessore della membrana

    if PROTO_TYPE > 3 or PROTO_TYPE < 1:
        print("Error: PROTO_TYPE must be 1, 2 or 3.")
        return
    ...
    model = protoZero(INPUT_FILE, TIME, POINTS, COEFF, MAX_LIPID, PROTO_TYPE, DIVISION, LIPID_EXP, RHO, DELTA, species, frequences, reactions, catalysis, events)
